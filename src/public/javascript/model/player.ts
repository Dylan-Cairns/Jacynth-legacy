...
type AiMoveSearchResultsObj = {
  ...
  tokenSpaceBonuses: number | undefined;
  log: string;
};
...
export class Player {
    ...
    if (space) {
      ...
      if (card) {
        ...
        if (this.gameType === 'singlePlayer') {
          ...
          const movesArr = JSON.parse(
            localStorage.getItem('movesArr') as string
          );
          movesArr.pop();
          localStorage.setItem('movesArr', JSON.stringify(movesArr));
        }
  ...
  placeToken = (spaceID: string) => {
    if (this.influenceTokens > 0) {
      ...
      if (this.gameType === 'singlePlayer') this.recordTokenPlay(spaceID);
    }
    return true;
  };
  ...
  public undoPlaceToken = (spaceID: string) => {
    ...
    if (this.gameType === 'singlePlayer') this.recordUndoTokenPlay(spaceID);
  };
...
export class Player_SinglePlayer extends Player {
  ...
  drawCard = () => {
    ...
    if (newCard) {
      ...
      if (this.gameType === 'singlePlayer') this.saveCardDrawToStorage(newCard);
      // send card to view
...
export class Player_ComputerPlayer extends Player_SinglePlayer {
  ...
  private getDistrictsGrowthPotential(boardSpace: BoardSpace) {
    ...
    return Number(
      (availableAdjSpaces.length * GROWTH_POTENTIAL).toPrecision(2)
    );
  }
      ...
      for (const suit of adjSuits) {
        if (cardSuits.includes(suit)) {
          ...
          if (adjControlPlayerID === 'Computer') {
            console.log(
              'intermediate scoring method, found adj card with same suit, but not controlled by computer. this message should never be seen. '
            );
            continue;
          }
  ...
  private extraPtforCardinHand(move: AiMoveSearchResultsObj) {
    ...
    if (points > 0) move.log += `same suit in hand + ${points} `;
  }
